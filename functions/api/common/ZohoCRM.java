...
public class ZohoCRM {
	...
	public JSONObject createRecord(String recordName, JSONObject record) throws Exception{
	...
	public JSONArray searchByQuery(String query) throws Exception {
		
		OkHttpClient client = new OkHttpClient();
		String url = "https://www.zohoapis.com/crm/v2/coql";
		JSONObject jsonBody = new JSONObject();
		jsonBody.put("select_query", query);		
		...
		try (Response res = client.newCall(req).execute()) {
			...
			if (code == 204){				
				...
				resultJson = (JSONArray)((JSONObject)new JSONParser().parse(body)).get("data");				
	...
	public JSONObject updateRecord(String recordName, String recordId , JSONObject record) throws Exception{
	...
	public JSONArray getRelatedRecords(String relatedName, String recordName, String recordId) throws Exception {
		OkHttpClient client = new OkHttpClient();
		String url = "https://www.zohoapis.com/crm/v2/" + recordName + "/" + recordId + "/" + relatedName;
		Request req = new Request.Builder().addHeader("Authorization", "Zoho-oauthtoken " + token).url(url).get().build();
		try (Response res = client.newCall(req).execute()) {
